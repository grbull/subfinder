...
import { RatedResult } from '../utils/bestMatch';

interface Props {
  options: RatedResult[];
  onSelect: (result: RatedResult) => void;
}

export function SelectResult({ onSelect, options }: Props): ReactElement {
    ...
    <>
      <Box flexDirection="column" height={5} width={80}>
        {options.slice(page * 5, page * 5 + 5).map((result, index) => (
          <Box height={1} key={result.link} width={80}>
